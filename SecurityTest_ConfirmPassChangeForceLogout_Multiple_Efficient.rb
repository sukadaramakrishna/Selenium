...
describe "SecurityTestAccessDenialToAnotherCommunity" do
  ...
  it "test_security_test_access_denial_to_another_community" do
	...
	#ary[i].find_element(:link, @config1['url'][i]).click

	if i == 0
	ary[i].find_element(:link, "Mission Hubs").click
	end
	if i == 1
	ary[i].find_element(:link, "Members").click
	end
	if i == 2
	ary[i].find_element(:link, "Groups and Lists").click
	end
	if i == 3
